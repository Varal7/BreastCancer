...
****************************************************************************/
    ...
    age = 1 * row['age'];        //   age
    dia = 1 * row['diameter'];   //   diameter in cm
    nnum = 1 * row['nodenumber'];//   nodenumber - if unknown, set to -1
    er = 1 * row['erstatus'];    //   erstatus - 0 unk - 1 pos - 2 neg
    pr = 1 * row['prstatus'];    //   prstatus - 0 unk - 1 pos - 2 neg
    her = 1 * row['her2status']; //   her2status - 0 unk - 1 pos - 2 neg
    grade = 1 * row['grade'];    //   grade  - 0 unk - 1 well diff - 2 moder diff - 3 poor diff
    his = 1 * row['histology'];  // histology
    /*
    0 - Unknown
    1 - Ductal
    2 - Lobular
...
****************************************************************************/
    ...
    /********************************************************************
    Initizes empty arrays necessary for calculations
    * L_breastcancer_percentage: contains the distribution of breast cancer lethality for 15 years
    * L_breastcancer_death_yearly: describes to the total number of deaths due to breast cancer at a single year
    ...
    * L_nonbreastcancer_prob: contains the probability of survival from the nsvr data adjusted to exclude the probability of dying from breast cancer as mentioned previously
    * L_nonbreastcancer_death_yearly: describes to the total number of deaths due to non-breast cancer causes at a single year
    ...
    * Variable       Without Therapy         With Therapy
...
***************************************************************************/
    //console.log("life exepct: " + Math.round(nvsr_life_expect[age]*10)/10)
    ...
    //console.log("mortality rate at 5 yrs stage (cancer): " + Math.round(L_cancer_death_cumm[5] * 1000)/10) + "%";
    //console.log("mortality rate at 5 yrs stage (overall): " + Math.round(L_overall_death_cumm[5] * 1000) / 10) + "%";
    return [L_cancer_death_cumm[5], L_overall_death_cumm[5]]
}
...
var output_name = process.argv[3];
...
.then((records) => {
    ...
    csvWriter.writeRecords(records.map((row) => Object.values(row)));
})
